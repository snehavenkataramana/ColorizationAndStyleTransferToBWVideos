...
def get_colorization_data(data_raw, opt, ab_thresh=5., p=.125, num_points=None):
    ...
    return add_color_patches_rand_gt(data, opt, p=p, num_points=0)

def get_colorization_data_with_ref(data_raw, ref_raw, opt, ab_thresh=5., p=.125, num_points=None):
    data = {}
    ...
    ref_lab = rgb2lab(ref_raw[0], opt)
    data['hint_B'] = ref_lab[:, 1:, :, :]
    data['mask_B'] = ref_lab[:, [0,], :, :]
...
def crop_mult(data,mult=16,HWmax=[800,1200]):
    ...
    # print('H, W', H, W, '; Hnew, Wnew', Hnew, Wnew, '; h, w', h, w)
    # print('h:h+Hnew,w:w+Wnew => ', h, ':' ,h+Hnew,w, ':', w+Wnew)
