...
def main():       
    ...
    directory = "examples/inputs/content_images"

    # print([f for f in os.listdir("examples/inputs/content_images") if fnmatch.fnmatch(f, '*.png')])
    ...
    #content_image = preprocess(params.content_image, params.image_size).type(dtype)
        print("Processing content image : " , filename)
        filename_1 = filename
        filename = directory+"/"+filename
        content_image = preprocess(filename,params.image_size).type(dtype)
        print("Processing content image : "+str(filename))
        ...
        def maybe_save(t,content):
            ...
            if should_save:
                ...
                if t == params.num_iterations:
                    filename = output_filename +content+ str(file_extension)
                else:
                    filename = str(output_filename) +content+ "_" + str(t) + str(file_extension)
        ...
        def feval():
            ...
            maybe_save(num_calls[0],filename_1)
...
class TVLoss(nn.Module):
    ...
    def forward(self, input):
        ...
        self.loss = self.strength * torch.sum((torch.abs(self.x_diff))) + (torch.sum(torch.abs(self.y_diff))) + (torch.sum(torch.abs(self.x_diff))-torch.sum(torch.abs(self.y_diff)))
        return input
